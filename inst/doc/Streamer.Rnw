...
We first load the \Rpackage{Rsamtools} and \Rpackage{Streamer}
packages.
...
\Rmethod{yield} method returns a \Rclass{GappedAlignments} object for
...
empty \Rclass{GappedAlignments} object will be returned.
    ...
    { 
       ...
       {    
            ...
            param <- ScanBamParam(which=.self$ranges[idx],
                                  what=character())
            aln <- readGappedAlignments(.self$file, param=param)
            seqlevels(aln) <- seq
            aln
       } else {
           GappedAlignments()
...
class calls the \Rmethod{countGenomicOverlaps} method with the
\Rclass{GappedAlignments} object, user supplied ranges and additional
arguments to control the behaviour of the
\Rmethod{countGenomicOverlaps} method.
...
.CountGOverlap$methods(
    ...
    {   
        ...
        {    
            ...
            olap <- countGenomicOverlaps(which, aln, type=.self$type, 
                                         resolution=.self$resolution,
                                         ignore.strand=.self$ignore.strand)
            df <- as(assays(olap)[[1]], "DataFrame")
            dimnames(df) <- 
                list(values(unlist(rowData(olap)))[["pos"]], "hits")
        } 
        df
    })

CountGOverlap <- 
    function(ranges, type = c("any", "start", "end", "within", "equal"),
